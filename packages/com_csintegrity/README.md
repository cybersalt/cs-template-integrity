...
```
com_csintegrity/
├── csintegrity.xml                                      # manifest
├── admin/
│   ├── services/provider.php                            # DI
│   ├── src/Extension/CsintegrityComponent.php           # component class
│   └── language/en-GB/
│       ├── com_csintegrity.ini
│       └── com_csintegrity.sys.ini
└── api/
    ├── src/
    ...
    └── language/en-GB/com_csintegrity.ini
```

## Install / test

Until there's a build script, zip the folder contents (everything inside `com_csintegrity/`, *not* the folder itself) and install via **System → Install → Upload Package File**.
...
curl -H "X-Joomla-Token: $TOKEN" \
     ...
     https://your-site.example/api/index.php/v1/csintegrity/overrides
```
...
- **ACL.** Component-level `core.manage` is implicitly required to hit the route. Finer-grained `csintegrity.view` permission comes in v1.0.
